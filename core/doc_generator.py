...
def _normalize_nested_list(item):
    if not isinstance(item, dict):
        return None
    return {
        "type": sanitize_inline_text(item.get("type") or "simple").lower(),
        "judul": sanitize_inline_text(item.get("judul", "")),
        "teks": sanitize_inline_text(item.get("teks", "")),
        "gambar_items": [
            {
                "type": "gambar",
                "imgId": sanitize_inline_text(image.get("imgId", "")),
                "data": image.get("data") or image.get("image") or image.get("src"),
        ...
        "anak": [
            nested_normalized
            for nested_normalized in (_normalize_list_wrapper(nested) for nested in item.get("anak", []))
...
def _normalize_content(content):
    ...
    normalized = {
        "type": content_type,
        "teks": sanitize_inline_text(content.get("teks", "")),
...
def _normalize_data(data: dict) -> dict:
    ...
    if not isinstance(kata_pengantar, dict):
        kata_pengantar = {}
    kata_pengantar["kata_pembuka"] = sanitize_inline_text(kata_pengantar.get("kata_pembuka", ""))
    kata_pengantar["kata_penutup"] = sanitize_inline_text(kata_pengantar.get("kata_penutup", ""))
    ...
    normalized["rujukan"] = [
        ...
        if isinstance(item, dict) and sanitize_inline_text(item.get("teks", ""))
    ]
...
def _split_bab_title(judul_bab: str, index: int):
    ...
    title = sanitize_inline_text(judul_bab)
    if " - " in title:
    ...
    if upper.startswith("BAB "):
        ...
        if len(parts) >= 2:
            ...
            body = parts[2].upper() if len(parts) > 2 else ""
            return label, body
    label = f"BAB {roman[index - 1] if index - 1 < len(roman) else index}"
    return label, upper
...
def _list_label(style: str, number: int) -> str:
    ...
    return mapping.get(style, f"{number}.")
...
class ProfessionalPKLGenerator:
    def __init__(self, data: dict):
        ...
        ensure_update_fields_on_open(self.document)
    ...
    def build_cover(self):
        title_specs = [
            ("LAPORAN", True, 20, 8),
            ("PRAKTIK KERJA INDUSTRI (PRAKERIN)", True, 18, 8),
    ...
    def build_body(self):
        ...
        for bab_index, bab in enumerate(self.data.get("isi_laporan", []), start=1):
            ...
            # UNIFIED HEADING: "BAB I PENDAHULUAN" dalam satu paragraf
            # menggunakan \n (soft break) agar di docx terlihat 2 baris
            # tapi di TOC terbaca 1 entri stabil.
            bab_text = f"{label_bab}\n{judul_bab}".strip().upper()
            self.toc.insert_heading(bab_text, level=1)
    ...
    def _render_list(self, lst: dict, level: int = 0):
        ...
        for index, item in enumerate(lst.get("items", []), start=1):
            ...
            title = item.get("judul", "") if lst_mode == "title" else ""
            text = item.get("teks", "")

            # Buat paragraf item list (yang mengandung label & title)
            paragraph = self.paragraphs.add_list_item(label, title, text, level=level)

            if title:
                # Jika ada judul, jadikan paragraf ini sebagai Heading 3
                # agar masuk ke TOC otomatis Word (o "1-3").
                self.toc.apply_heading_style(paragraph, level=3)
